...
public class MustAcceptTermsAllowlistTests
{
    private static async Task<(int statusCode, bool nextCalled)> RunAsync(string path, bool mustAccept = true)
    {
        ...
        var middleware = new MustAcceptTermsMiddleware(next);
    ...
    [Fact]
    public async Task MustAcceptFalse_AnythingPasses()
    {
        var (status, nextCalled) = await RunAsync("/api/v1/contacts", mustAccept: false);
        status.Should().Be(200);
        nextCalled.Should().BeTrue();
    }
}
